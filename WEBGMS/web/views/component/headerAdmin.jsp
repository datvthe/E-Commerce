<%-- 
    ...
    Created on : Sep 21, 2025, 7:50:20 AM
    Author     : trant
--%>
...
<html>
    ...
    <body>
        <%-- Compute admin notification data in server-side Java to avoid EL instantiation --%>
        <%
            model.user.Users __u = (model.user.Users) session.getAttribute("user");
            Integer __unread = 0;
            java.util.List<model.Notifications> __notifs = java.util.Collections.emptyList();
            if (__u != null) {
                service.NotificationService __svc = new service.NotificationService();
                try {
                    __unread = __svc.getUnreadCount(__u.getUser_id());
                    __notifs = __svc.getUserNotifications(__u.getUser_id());
                } catch (Exception ignore) {}
            }
            request.setAttribute("_unread", __unread);
            request.setAttribute("_notifs", __notifs);
        %>
        <!-- ========== header start ========== -->
            <header class="header">
                            ...
                            <div class="header-right">
                                ...
                                <div class="notification-box ml-15 d-none d-md-flex">
                                    <button class="dropdown-toggle" type="button" id="notification" data-bs-toggle="dropdown"
                                        ...
                                        <span><c:out value="${_unread}"/></span>
                                    </button>
                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="notification" style="min-width: 380px;">
                                        <c:choose>
                                            <c:when test="${sessionScope.user != null}">
                                                <c:forEach var="n" items="${_notifs}" begin="0" end="9">
                                                    <li>
                                                        <a href="<c:url value='/admin/orders'/>" style="text-decoration:none;">
                                                            <div class="content" style="padding:8px 12px;">
                                                                <h6 class="mb-1">
                                                                    <c:out value="${n.title}"/>
                                                                    <span class="text-regular">- <c:out value="${n.type}"/></span>
                                                                </h6>
                                                                <p class="mb-1" style="white-space:normal;">
                                                                    <c:out value="${n.message}"/>
                                                                </p>
                                                                <span class="text-muted"><c:out value="${n.createdAt}"/></span>
                                                            </div>
                                                        </a>
                                                    </li>
                                                </c:forEach>
                                                <c:if test="${empty _notifs}">
                                                    <li><div class="px-3 py-2 text-muted">Không có thông báo</div></li>
                                                </c:if>
                                            </c:when>
                                            <c:otherwise>
                                                <li><div class="px-3 py-2 text-muted">Chưa đăng nhập</div></li>
                                            </c:otherwise>
                                        </c:choose>
                                    </ul>
                                </div>
